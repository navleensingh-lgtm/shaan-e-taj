      ...
      {mainCategories.length > 0 && (
        <div className="mt-6 flex flex-wrap gap-2">
          {mainCategories.map((c) => (
            <a
              key={c.slug}
              href={`/collections/${c.slug.toLowerCase()}`}
              className="rounded-sm border border-brand-border px-3 py-1.5 text-[10px] uppercase tracking-wider text-brand-muted hover:border-rose hover:text-rose-dark"
            >
              {c.name}
            </a>
          ))}
