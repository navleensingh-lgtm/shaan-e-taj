...
export default async function ProductPage({
  ...
  const primary =
    sortedImages.find((i: { isPrimary?: boolean }) => i.isPrimary) ?? sortedImages[0];
  const gallery = sortedImages.filter((i: { url: string }) => i.url !== primary?.url);
    ...
    <section className="mx-auto max-w-6xl px-5 py-12 lg:px-8">
      <div className="grid gap-10 lg:grid-cols-2">
        ...
        <div>
          ...
          <p className="mt-3 text-2xl font-medium text-rose-dark">₹{price.toLocaleString("en-IN")}</p>
