...
export function OrderPricingSummary({
  ...
  const stitchingLabel =
    qty > 1 && perUnit > 0
      ? `Stitching (${formatPrice(perUnit)} × ${qty})`
      : "Stitching";
  ...
  return (
    <div className={`space-y-2 ${row} text-brand-muted`}>
      <div className="flex justify-between">
        ...
        <span className="text-brand-text">{formatPrice(subtotalPaise)}</span>
      </div>
      {stitchingType === "FULLY_STITCHED" && (
        <div className="flex justify-between">
          <span>{stitchingLabel}</span>
          <span className="text-brand-text">
            {stitchingPaise > 0 ? formatPrice(stitchingPaise) : "Included"}
          </span>
        </div>
      )}
      <div className="flex justify-between">
        <span>Shipping</span>
        <span className="text-brand-text">
          {shippingPaise > 0 ? formatPrice(shippingPaise) : "Free"}
        </span>
      </div>
      <div
